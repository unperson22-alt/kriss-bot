...
async def handle_task(request):
    ...
    try:
        ...
        response = await process(message, user_id,
                                 ...
                                 short=is_banter)
        _res = None
        # Одно правило на всех семи (shared/banter.should_post_to_group):
        # постим, если это болталка ИЛИ вызывающий явно попросил. Правил было
        # четыре, и на пути «бот→Филли→бот» четверо безусловных постили в
        # группу личный ответ и задваивали ленту (14.09.2026).
        if _banter.should_post_to_group(data):
            # Имя НЕ приписываем: Telegram и так рисует его над сообщением,
            # и 13.09.2026 в группе вышло «Крис:\nГослинг, "на месте" — это
